...
(function(){
  ...
  function rBike(g,x,y,w,down,swinging,col,lean,braking){
    ...
    // wheels
    g.fillStyle = '#23262c';
    g.beginPath(); g.ellipse(-w*0.02, -h*0.06, w*0.16, h*0.17, 0, 0, 7); g.fill();
    // body
    g.fillStyle = col;
    g.beginPath();
    ...
    if(swinging){
      ...
      g.lineCap = 'round';
      g.beginPath();
      g.moveTo(w*0.10, -h*0.46);
      g.lineTo(w*0.46, -h*0.40);
      g.stroke();
    }
    g.restore();
  }
  ...
  function bindInput(){
    document.addEventListener('keydown', function(e){
      ...
      var hit = true;
      if(k==='ArrowLeft')       set('left',true);
      else if(k==='ArrowRight') set('right',true);
    ...
    document.addEventListener('keyup', function(e){
      ...
      if(g !== 'rash' && g !== 'belter') return;
      var k = e.key, set = g === 'rash' ? AG3.rashKey : AG3.belterKey;
      if(k==='ArrowLeft') set('left',false);
      if(k==='ArrowRight') set('right',false);
      if(k==='ArrowUp') set('up',false);
      if(k==='ArrowDown') set('down',false);
